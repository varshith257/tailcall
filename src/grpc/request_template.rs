...
mod tests {
    ...
    use crate::config::{Config, Field, GraphQLOperationType, Grpc, Link, LinkType, Resolver, Type};
    ...
    async fn get_protobuf_op() -> ProtobufOperation {
        ...
        config.types.insert(
            "foo".to_string(),
            Type::default().fields(vec![("bar", Field::default().resolvers(vec![Resolver::Grpc(grpc)]))]),
        );
