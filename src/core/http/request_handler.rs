...
async fn handle_origin_tailcall<T: DeserializeOwned + GraphQLRequestLike>(
    ...
    handle_request_with_custom_origin::<T>(req, app_ctx, request_counter, "https://tailcall.run").await
}

async fn handle_request_with_custom_origin<T: DeserializeOwned + GraphQLRequestLike>(
    req: Request<Body>,
    app_ctx: Arc<AppContext>,
    request_counter: &mut RequestCounter,
    origin: &str,
...
async fn handle_rest_apis(
    ...
    *request.uri_mut() = request.uri().path().replace(API_URL_PREFIX, "").parse()?;
    ...
    if let Some(p_request) = app_ctx.endpoints.matches(&request) {
        ...
        return async {
            let graphql_request = p_request.into_graphql_request();
            let response = graphql_request.data(req_ctx.clone()).execute(&app_ctx.schema).await;
            let mut resp = update_cache_control_header(response, &app_ctx, req_ctx.clone()).into_response()?;
            update_response_headers(&mut resp, &req_ctx, &app_ctx);
            Ok(resp)
        }
        .instrument(span)
...
async fn handle_request_inner<T: DeserializeOwned + GraphQLRequestLike>(
    ...
    match *req.method() {
        ...
        hyper::Method::POST => match req.uri().path() {
            "/graphql" => graphql_request::<T>(req, app_ctx.as_ref(), req_counter).await,
            "/showcase/graphql" if app_ctx.blueprint.server.enable_showcase => {
                let app_ctx = match showcase::create_app_ctx::<T>(&req, app_ctx.runtime.clone(), false).await? {
                    Ok(app_ctx) => app_ctx,
        ...
        hyper::Method::GET => {
            if let Some(TelemetryExporter::Prometheus(prometheus)) = app_ctx.blueprint.telemetry.export.as_ref() {
...
pub async fn handle_request<T: DeserializeOwned + GraphQLRequestLike>(
    ...
    let cors_config = app_ctx.blueprint.server.cors.as_ref();
    let response = if let Some(origin) = req.headers().get(&header::ORIGIN) {
        if origin == TAILCALL_HTTPS_ORIGIN || origin == TAILCALL_HTTP_ORIGIN {
            handle_origin_tailcall::<T>(req, app_ctx, &mut req_counter).await
        } else {
            handle_graphql_request::<T>(req, app_ctx, &mut req_counter, cors_config).await
        }
    } else {
        handle_graphql_request::<T>(req, app_ctx, &mut req_counter, cors_config).await
    };
